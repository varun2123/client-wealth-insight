...
export const PositionsTable = ({ positions }: PositionsTableProps) => {
  ...
  const fluctuation = (Math.random() * 5 + 5) * (Math.random() > 0.5 ? 1 : -1); 
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border hover:bg-muted/50">
                ...
                <TableHead className="text-right text-muted-foreground">Daily MTM</TableHead>
                ...
                <TableRow key={position.id} className="border-border hover:bg-muted/50">
                  ...
                  <TableCell className="text-right">
                    <div className={`flex items-center justify-end gap-1 ${
                      (position.quantity * position.currentPrice) - (position.quantity * position.buyPrice)>= 0 ? 'text-success' : 'text-destructive'
                    }`}>
                      {(position.quantity * position.currentPrice) - (position.quantity * position.buyPrice) >= 0 ? (
                        <TrendingUp className="w-3 h-3" />
                      ) : (
                        <TrendingDown className="w-3 h-3" />
                      )}
                      {formatCurrency((position.quantity * position.currentPrice) - (position.quantity * position.buyPrice))}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <span className={
                      (((position.quantity * position.currentPrice) - (position.quantity * position.buyPrice))/(position.quantity * position.buyPrice)) * 100 >= 0 ? 'text-success' : 'text-destructive'
                    }>
                      {formatPercent((((position.quantity * position.currentPrice) - (position.quantity * position.buyPrice))/(position.quantity * position.buyPrice)) * 100)}
                    </span>
                  </TableCell>
